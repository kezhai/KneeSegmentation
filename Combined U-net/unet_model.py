...
def create_unet_model2D(input_image_size,output_image_size,
                        n_labels=1,
                        layers=4,
    ...
    if mode == 'classification':
        if number_of_classification_labels == 1:
            ...
            outputs = Reshape((output_image_size,2))(outputs)
        ...
        if number_of_classification_labels == 1:
            ...
            unet_model.compile(loss='categorical_crossentropy',
                                optimizer=opt.Adam(lr=init_lr), metrics=['accuracy', 'categorical_crossentropy'],sample_weight_mode="temporal")
...
def create_unet_model3D(input_image_size,output_image_size,
                        n_labels=1,
                        layers=4,
                        lowest_resolution=16,
    ...
    encoding_convolution_layers = []
    pool = None
#    reshape = Reshape((16,64,32,1))(inputs)
    for i in range(len(layers)):
        number_of_filters = lowest_resolution * 2**(layers[i])
        if i == 0:
